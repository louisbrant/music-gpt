...
return (
    <VStack spacing={24} align="start" bg="#ffffff" minH="100vh" pt={24}>
      ...
      {/* Benefits Section */}
      <Box w="full" py={12} px={12} bg="#f2f2f2" borderRadius="xl" shadow="lg">
        <MotionHeading as="h3" fontWeight="bold" fontSize="5xl" color="#000000" letterSpacing="tight">
          {specificService}
        </MotionHeading>
        <SimpleGrid mt={6} columns={{ base: 2, md: 4 }} gap={8}>
          {relatedVisuals.map((visual, index) => (
            <MotionBox
              key={`visual-${index}`}
              borderRadius="xl"
              overflow="hidden"
              boxShadow="md"
              whileHover={{ scale: 1.1, boxShadow: "2xl" }}
              transition={{ duration: 0.3 }}
            >
              <MotionImage src={visual} alt={`Related visual ${index}`} layout="responsive" width="100%" height="100%" objectFit="cover" />
            </MotionBox>
          ))}
        </SimpleGrid>
        <MotionText fontWeight="medium" fontSize="2xl" color="#000000" mt={6} letterSpacing="wider" lineHeight="tall">
          {shortDescription}
        </MotionText>
        <MotionButton variant="solid" bg="green.500" mt={5} color="white" py={2} px={8} fontSize="xl" fontWeight="bold" shadow="md" _hover={{ shadow: "lg" }}>
          Book Now
        </MotionButton>
      </Box>
